...
class HeliocentricSolarWidget(AbsWidget):
    @property
    ...
    def __init__(self, hardware, position, size,
        ...
        self.colors.add_color('PLANET_TEXT', foreground_color)
    ...
    def _draw_full(self):
        self.display.text("Heliocentric", 30, 100, self.colors['FG'], v_spacing=-2, font_name='font_krungthep_14.bin')
        pass

    def _assign_planetary_subwidgets(self):
        planets = [p for p in (self.CLASSICAL_PLANETS if self.classical else self.ALL_PLANETS) if p != 'Sun']
        j2k = self.hardware.rtc.earth_time.j2kdelta
        planet_angles = []
        for planet in planets:
            e = Ephemeris(planet, j2k)
        ...
        self.add_subwidget(SymbolWidget(self.hardware, (self.xo-4, self.yo-4), (8,9), 
                    'Sun', c=self.colors.get_color_name('PLANET_TEXT'), 
                    bg=self.colors.get_color_name('PLANET'), ))
