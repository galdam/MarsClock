...
#from upydrivers.display.font.fontmanager import FontManager
...
class AbstractUPyDisplayDevice(framebuf.FrameBuffer): #, metaclass=ABCMeta):
    ...
    def __init__(self, width: int, height: int,
        ...
        print(f"[DisplayDevice.init] Size: {self.width} x {self.height}")
        self._buffer = bytearray(self.height * math.ceil(self.width / (8 // self.palette.nbits)))
        #self._font_manager = FontManager(display_device=self)
        self._has_full_updates = True
        self._has_partial_updates = True
        print(f"[DisplayDevice.init] Buffer len: {len(self._buffer)} ({self.height} * {math.ceil(self.width / (8 // self.palette.nbits))}).")
        print(f"[DisplayDevice.init] Palette nBits: {self.palette.nbits}")
    ...
    def font_text(self, s, x, y, c, centered=False, bg=None, font=None, size=None):
        raise NotImplementedError()
        #self.text
        #self._font_manager.text(s, x, y, c, centered=centered, bg=bg, font=font, size=size)
