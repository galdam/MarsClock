...
class FrameBuffer:
    ...
    def ellipse(self, x, y, xradius, yradius, col, f=False, m=None):
        """
        x: x center
        y: y center
        xr: x radius
        ...
        def draw_ellipse_points(fb, cx, cy, x_counter, y_counter, color, mask):
            if (mask & ELLIPSE_MASK_FILL):
                if (mask & ELLIPSE_MASK_Q1):
                    fb.fill_rect(cx, cy - y_counter, x_counter + 1, 1, color)
                if (mask & ELLIPSE_MASK_Q2):
                    fb.fill_rect(cx - x_counter, cy - y_counter, x_counter + 1, 1, color)
                if (mask & ELLIPSE_MASK_Q3):
                    fb.fill_rect(cx - x_counter, cy + y_counter, x + 1, 1, color)
        ...
        # mp_int_t args[5];
        # framebuf_args(args_in, args, 5); // 0: cx, 1: cy, 2: xradius, 3: yradius, 4: col 5: f, 6: m
        ...
        if (xradius == 0 & yradius == 0):
            self.pixel(cx, cy, col, mask & ELLIPSE_MASK_ALL)
            return
        ...
        while (stoppingx >= stoppingy):  # 1st set of points,  y' > -1
            self.draw_ellipse_points(cx, cy, x_counter, y_counter, col, mask)
            y_counter += 1
