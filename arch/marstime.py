...
DAYS = {
    'Utopian': {
        0: 'Lunasol', 1: 'Earthsol', 2: 'Mercurisol', 3: 'Jupitersol',
        4: 'Venusol', 5: 'Saturnsol', 6:'Sunsol', },
    'Darian': {
        0: 'Saturni', 1: 'Solis', 2: 'Lunae', 3: 'Martis',
        4: 'Mercurii', 5: 'Jovis', 6: 'Veneris', },
    'English': {
...
def make_struct_time(tm_year, tm_mon, tm_mday, tm_hour=0, tm_min=0, tm_sec=0, tm_wday=0, tm_yday=1, *args):
    #return time.struct_time((tm_year, tm_mon, tm_mday, tm_hour, tm_min, tm_sec, 0, 1, -1))
    return (tm_year, tm_mon, tm_mday, tm_hour, tm_min, tm_sec, tm_wday, tm_yday, -1)
...
class EarthCal:
    ...
    """ 
    @staticmethod
    def local_time(tz_hours, et_gmt=None):
        et_gmt_secs = time.mktime(time.gmtime() if et_gmt is None else et_gmt)
        et_local_secs = et_gmt_secs + (tz_hours * (60 * 60))
        time.localtime(et_local_secs)
    """

    @staticmethod
    def apply_time_modifier(earth_dt, day=0, hour=0, minute=0, second=0):
        dt_sec = time.mktime(tuple(list(earth_dt[:6]) + [-1, -1]))
        dt_sec += (day * 86400) + (hour * 3600) + (minute * 60) + second
        return make_datetime_tup(*time.gmtime(dt_sec))

    @staticmethod
    def recalc_timetup(earth_dt):
        return time.gmtime(time.mktime(earth_dt))

    @staticmethod
    def last_sunday_of_month(year, month):
        if month not in [3, 10]:
            raise NotImplementedError("Support is only currently for March (3) and October (10).")
        day = 31
        dt = EarthCal.recalc_timetup((year, month, day, 0, 0, 0, -1, -1))
        if dt[6] == 6:
            last_sunday = dt
        else:
            last_sunday = EarthCal.apply_time_modifier(dt, day=-1 * (1 + dt[6]))
        return last_sunday

    @staticmethod
    def is_bst(earth_dt):
        year, month, day = earth_dt[0:3]
        if month < 3 or month > 10:
            return False
        if month > 3 and month < 10:
            return True
        assert month in [3, 10], f"What month is this? {month}"
        last_sunday = EarthCal.last_sunday_of_month(year, month)
        # print(last_sunday)
        return (day >= last_sunday[2]) if month == 3 else (day < last_sunday[2])
